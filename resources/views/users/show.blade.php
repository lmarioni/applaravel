...
<a href="/{{ $user->username }}/followers" class="btn btn-link">
  Seguidores <span class="badge badge-default">
...
@if(Auth::check())
 @if(Gate::allows('dms',$user))
  <form class="" action="/{{ $user->username }}/dms" method="post">
    ...
    <button type="submit" name="button" class="btn btn-success">Enviar DM</button>
  </form>
  @endif

@if(Auth::user()->isFollowing($user))
<form class="" action="/{{ $user->username }}/unfollow" method="post">
  {{ csrf_field() }}
  @if(session('success'))
